...
function TrendDriversSection({ standings }) {
  ...
  const trendingDrivers = standings
    ...
    .map((driver) => {
      if (driver.driver_number === 12) return { ...driver, title: "Race Winner",                   description: "First victory in Monaco" };
      if (driver.driver_number === 14) return { ...driver, title: "Back to top ten",               description: "Managed to score a point" };
      if (driver.driver_number === 6)  return { ...driver, title: "First podium with Red Bull Racing", description: "Returns to the top three with the big team" };
      return driver;
    });
